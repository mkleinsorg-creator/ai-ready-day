# AI Ready Day · Setup für Windows (PowerShell)
# Ausführen mit:
...
Write-Host "  AI Ready Day · Setup für deinen Windows-PC" -ForegroundColor White
Write-Host "════════════════════════════════════════════════" -ForegroundColor White
Write-Host ""
...
# Prüfen: winget verfügbar?
if (-not (Get-Command winget -ErrorAction SilentlyContinue)) {
    Err "winget (Windows Package Manager) ist nicht installiert."
    Write-Host "    Bitte den Microsoft Store öffnen und 'App Installer' aktualisieren," -ForegroundColor Yellow
...
if (-not (Get-Command gh -ErrorAction SilentlyContinue)) {
    Write-Host "    Ich installiere die GitHub-CLI via winget (damit du später ohne Passwort-Eingabe pushen kannst)..."
...
if ($LASTEXITCODE -ne 0) {
    ...
    Write-Host "    Wähl: 'GitHub.com' → 'HTTPS' → 'Login with a web browser'." -ForegroundColor Yellow
    Write-Host "    Code im Terminal kopieren, im Browser einfügen, autorisieren." -ForegroundColor Yellow
    Write-Host ""
    & gh auth login -h github.com -p https -w
    if ($LASTEXITCODE -ne 0) {
        Say "GitHub-Login übersprungen — du kannst das später mit 'gh auth login' nachholen."
    }
} else {
...
Write-Host ""
Write-Host "════════════════════════════════════════════════" -ForegroundColor White
Write-Host "  ✅ Fertig! Du bist startklar." -ForegroundColor Green
Write-Host "════════════════════════════════════════════════" -ForegroundColor White
Write-Host ""
Write-Host "  Was du JETZT machst:" -ForegroundColor White
Write-Host ""
Write-Host "    1. Dieses PowerShell-Fenster liegen lassen — wir brauchen es vielleicht später." -ForegroundColor White
Write-Host ""
Write-Host "    2. Öffne die Claude Desktop App (die du sonst auch nutzt)." -ForegroundColor White
Write-Host ""
Write-Host "    3. Drück Ctrl + 3 — das öffnet das integrierte" -ForegroundColor White
Write-Host "       Claude-Code-Fenster innerhalb der App." -ForegroundColor White
Write-Host "       (Falls Ctrl + 3 nicht klappt, frag Max — das Tastenkürzel" -ForegroundColor White
...
Write-Host ""
Write-Host "         Lies @$TARGET_DIR\prompts\00-router.md vollständig" -ForegroundColor Cyan
Write-Host "         und folge diesen Anweisungen als Master-Prompt." -ForegroundColor Cyan
Write-Host ""
Write-Host "  Ab da führt dich Claude durch BMAD und den Rest des Tages." -ForegroundColor White
Write-Host ""
Write-Host "════════════════════════════════════════════════" -ForegroundColor White
Write-Host ""
